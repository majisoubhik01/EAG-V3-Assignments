...
async def _crawl4ai_fetch(url: str) -> dict:
    ...
    def _fetch():
        ...
        with urllib.request.urlopen(req, timeout=20.0) as response:
            return response.read().decode("utf-8", errors="replace"), response.status, response.headers.get("content-type", "text/plain")

    try:
        html, status, content_type = await asyncio.to_thread(_fetch)
        text = trafilatura.extract(html) or "Failed to extract text from HTML."
        ...
        return {
            ...
            "length_bytes": len(str(e).encode("utf-8")),
